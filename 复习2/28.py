...
for chunk in graph.stream({"messages":["小米的销售情况怎么样"]},stream_mode="values"):
    chunk['messages'][-1].pretty_print()
    
for chunk in graph.stream({"messages":["帮我分析一下数据库中北京， 天津，大连的天气，做一个详细的对比，并生成出现建议"]},stream_mode="values"):
    chunk['messages'][-1].pretty_print()
