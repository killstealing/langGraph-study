...
builder.add_node('execute_risk_function',execute_risk_function)
builder.add_node('execute_normal_function',tool_node)
...
builder.add_edge('execute_normal_function','call_model')
builder.add_edge('execute_risk_function','call_model')
builder.add_edge('call_model',END)

graph=builder.compile(checkpointer=memory,interrupt_before=['execute_risk_function'])

    
config ={"configurable":{"thread_id":"12311"}}

for chunk in graph.stream({"messages":["帮我删除数据库中大连的天气数据"]},config=config,stream_mode="values"):
    ...
    if state.tasks[0].name=='execute_risk_function':
        while True:
            user_input=input("是否允许执行删除操作？请输入 '是'或者 '否'")
        ...
        if user_input =='是':
            ...
            graph.update_state(config,{'messages':[new_message]},as_node="execute_risk_function")
            for event in graph.stream(None,config=config,stream_mode="values"):
                event['messages'][-1].pretty_print()
...
config ={"configurable":{"thread_id":"12311"}}
run_multi_round_dialog(graph,config)
...
async def main1():
    ...
    async for chunk in graph.astream({"messages":["你好，查询一下天津天气，保存到数据库"]},config=config,stream_mode="values"):
        chunk['messages'][-1].pretty_print()
        
    async for chunk in graph.astream({"messages":["将天津从数据库中删掉"]},config=config,stream_mode="values"):
        chunk['messages'][-1].pretty_print()

    snapshot=graph.get_state(config)
    print('snapshot ',snapshot.values)
